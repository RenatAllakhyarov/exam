...
const PostCard = ({ post }: IPostCardProps): ReactElement => {
    ...
    const shortBody = post.body.length > 100
    ...
    return (
        <div className="post-card">
            ...
            <p className="post-card-author secondary-text">Автор: {authorName}</p>
            <p className="post-card-body secondary-text">{shortBody}</p>
            <button onClick={handleOpenPost} className="open-post-button nav-button">
                Открыть
            </button>
        </div>
    );
